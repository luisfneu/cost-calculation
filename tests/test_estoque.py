...
def test_inventario_ajusta_e_define_minimo(client, app, seed):
    ...
    pid = seed["peca"]
    client.post("/estoque/inventario", data={
...
def test_reserva_reduz_disponivel(client, app, seed):
    ...
    client.post(f"/pecas/{pid}/estoque/reservar", data={"tamanho": "P", "quantidade": "2"})
    with app.app_context():
        p = Peca.query.get(pid)
...
def test_reserva_nao_excede_estoque(client, app, seed):
    from app.models import Peca
    pid = seed["peca"]
    r = client.post(f"/pecas/{pid}/estoque/reservar",
...
def test_liberar_reserva(client, app, seed):
    from app.models import Peca
    pid = seed["peca"]
    client.post(f"/pecas/{pid}/estoque/reservar", data={"tamanho": "P", "quantidade": "3"})
    client.post(f"/pecas/{pid}/estoque/liberar-reserva", data={"tamanho": "P", "quantidade": "1"})
    with app.app_context():
...
def test_venda_usa_disponivel_no_json(client, app, seed):
    pid = seed["peca"]  # P=5
    client.post(f"/pecas/{pid}/estoque/reservar", data={"tamanho": "P", "quantidade": "2"})
    body = client.get("/vendas/nova").get_data(as_text=True)
