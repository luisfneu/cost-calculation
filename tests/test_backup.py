...
def test_restore_arquivo_invalido(app, seed):
    c = _admin(app)
    r = c.post("/console/erp/backup/restaurar",
...
def test_restore_valido(app, seed):
    c = _admin(app)
    dbbytes = c.get("/console/erp/backup").data  # backup íntegro do próprio banco de teste
    r = c.post("/console/erp/backup/restaurar",
...
def test_restore_exige_admin(app, seed):
    c = app.test_client()
    c.post("/console/erp/login", data={"senha": "test"})  # logado, mas não admin
    with c.session_transaction() as s:
        s["admin"] = False
    dbbytes = None
    # gera um .db qualquer válido a partir do backup admin de outro client
    admin = _admin(app)
    dbbytes = admin.get("/console/erp/backup").data
    r = c.post("/console/erp/backup/restaurar",
               data={"arquivo": (io.BytesIO(dbbytes), "bkp.db")},
               content_type="multipart/form-data", follow_redirects=True)
    assert r.status_code == 200
