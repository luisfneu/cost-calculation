...
def test_login_senha_mestre_define_admin(client):
    ...
    r = client.get("/console/erp/usuarios")  # rota só de admin
    assert r.status_code == 200


def test_criar_usuario_e_login(app, client):
    from app.models import Usuario
    r = client.post("/console/erp/usuarios/novo", data={
    ...
    c2 = app.test_client()
    r = c2.post("/console/erp/login", data={"login": "sabrina", "senha": "1234"}, follow_redirects=True)
    assert r.status_code == 200
    assert c2.get("/console/erp/").status_code == 200
    # não é admin: gestão de usuários é bloqueada (redireciona)
    assert c2.get("/console/erp/usuarios", follow_redirects=False).status_code == 302


def test_login_senha_errada_do_usuario(app, client):
    client.post("/console/erp/usuarios/novo", data={"nome": "X", "login": "x", "senha": "certa"})
    c2 = app.test_client()
    c2.post("/console/erp/login", data={"login": "x", "senha": "errada"})
    assert c2.get("/console/erp/", follow_redirects=False).status_code == 302  # não logou
...
def test_auditoria_registra_login_e_venda(client, app, seed):
    ...
    assert client.get("/console/erp/auditoria").status_code == 200
