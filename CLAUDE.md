...
- Ícones de tela inicial: gerados por `gerar_icones.py` (monograma + `qlmanage` + Pillow) — Loja mocha, ERP ink com selo `ERP`. **Ícone editado à mão nunca é sobrescrito**: o script compara o sha256 com `icones-gerados.json` e pula o que não bater (`--forcar` ignora a proteção). Hoje `apple-touch-icon.png` e `apple-touch-icon-erp.png` são arte manual. Favicons separados: loja usa `favicon.ico` (claro), ERP usa `favicon-erp.ico` (escuro) — para distinguir as abas. PNG do `apple-touch-icon` fica **quadrado e opaco** (iOS arredonda sozinho; canto transparente vira preto); o arredondado vem da moldura interna. Manifests separados: `manifest.webmanifest` (loja, scope `/`) e `manifest-erp.webmanifest` (scope `/console/erp/`).
